...
namespace Input {
    ...
    /* Pre: minimum <= maximum, query and error are the displayed messages */
    /* Post: returns an int (n) entered by the user, minimum <= n <= maximum, which is not in the banned set  */
    int read_int(int minimum, int maximum, const std::set<int>& banned, const std::string& query, const std::string& err);
